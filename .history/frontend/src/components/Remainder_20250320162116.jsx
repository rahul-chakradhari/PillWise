...
const Remainder = () => {
  const [remainders, setRemainders] = useState([]); // Initialize with an empty array
  ...
  useEffect(() => {
    const savedRemainders =
      JSON.parse(localStorage.getItem("allRemainders")) || []; // Corrected line
    setRemainders(savedRemainders);
  }, []);

  // Save remainders to local storage
  const saveToLocalStorage = (newRemainders) => {
    localStorage.setItem("allRemainders", JSON.stringify(newRemainders));
  };
